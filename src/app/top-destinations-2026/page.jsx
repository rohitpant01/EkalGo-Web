      ...
      <section className="relative pt-40 pb-24 overflow-hidden border-b border-slate-100 bg-slate-50/50">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center space-y-8">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-primary-50 border border-primary-200">
            ...
            <span className="text-xs font-bold uppercase tracking-[0.2em] text-primary-600">Official 2026 Rankings</span>
          </div>
          
          <h1 className="text-5xl md:text-8xl font-display font-bold tracking-tight text-slate-900">
            Best of <span className="text-gradient-primary italic">India.</span>
          </h1>
          ...
          <div className="flex justify-center gap-12 pt-8 border-t border-slate-100 max-w-lg mx-auto">
             <div className="text-center">
                <div className="text-3xl font-bold font-display text-slate-900">30+</div>
                <div className="text-[10px] text-slate-400 uppercase tracking-widest mt-1">Cites Vetted</div>
             </div>
             <div className="text-center">
                <div className="text-3xl font-bold font-display text-slate-900">1.2K</div>
                <div className="text-[10px] text-slate-400 uppercase tracking-widest mt-1">Gems Found</div>
             </div>
             <div className="text-center">
                <div className="text-3xl font-bold font-display text-slate-900">4.9/5</div>
                <div className="text-[10px] text-slate-400 uppercase tracking-widest mt-1">AI Precision</div>
             </div>
          </div>
        </div>
      </section>
          ...
          <div key={cat.id} className="space-y-16">
            <div className="flex items-end justify-between border-b border-slate-100 pb-8">
              <div className="space-y-4">
                <div className="flex items-center gap-3 text-2xl font-display font-bold uppercase tracking-widest text-slate-900">
                  {cat.icon}
                  {cat.title}
                </div>
              ...
              <div className="text-6xl font-display font-black text-slate-100 select-none">
                0{catIdx + 1}
              </div>
            </div>
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-8">
              {cat.items.map((item, idx) => (
                <Link 
                  ...
                  className="group relative block aspect-[3/4] rounded-[2.5rem] overflow-hidden border border-slate-100 hover:border-primary-200 transition-all duration-700 shadow-soft hover:shadow-card bg-white"
                >
                  <div className="absolute inset-0">
                    {item.image ? (
                      ...
                      <div className="w-full h-full bg-slate-50 flex items-center justify-center">
                        <MapPin size={48} className="text-slate-200 group-hover:scale-125 group-hover:text-primary-100 transition-all duration-700" />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-8 space-y-4 transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                    ...
                    <p className="text-xs text-white/70 line-clamp-2 opacity-0 group-hover:opacity-100 transition-opacity duration-700 delay-100">
                      {item.tagline}
                    </p>
                    
                    <div className="flex items-center gap-2 text-primary-400 text-xs font-bold pt-2">
                       Explore More <ArrowRight size={14} className="group-hover:translate-x-2 transition-transform" />
                    </div>
                  </div>

                  <div className="absolute top-6 left-6 p-2 rounded-xl bg-white/20 backdrop-blur-md border border-white/30 opacity-0 group-hover:opacity-100 transition-opacity duration-500">
      ...
      <section className="py-32 bg-slate-50 border-y border-slate-100">
         <div className="max-w-5xl mx-auto px-6">
            <div className="text-center space-y-4 mb-20">
               <h2 className="text-3xl md:text-5xl font-display font-bold text-slate-900">The EkalGo <span className="text-gradient-primary">Travel Index</span></h2>
               <p className="text-slate-500">Aggregated comparison metrics for top destinations.</p>
            </div>
                        ...
                        {destinations.slice(0, 8).map((d, i) => (
                           <tr key={i} className="hover:bg-slate-50 transition-colors group">
                              <td className="px-8 py-6">
                                 <Link href={`/explore/${d.slug}`} className="font-bold flex items-center gap-3 text-slate-900 hover:text-primary-600 transition-colors">
                                    <MapPin size={14} className="text-primary-400 group-hover:text-primary-600 transition-colors" />
                                    {d.name}
                                 </Link>
                              </td>
                              <td className="px-8 py-6">
                              ...
                              <td className="px-8 py-6 text-slate-500">{d.category}</td>
                              <td className="px-8 py-6 text-slate-500">{d.best_time}</td>
                           </tr>
                        ))}
                     </tbody>
                  </table>
